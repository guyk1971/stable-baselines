...
def run_experiment(experiment_params):
    ...
    # make the env
    env_id = CC_ENVS.get(experiment_params.env_id,None)
    ...
    # create the agent
    # if theres a trained agent - check that its a valid file name
    ...
    algo_=agent_hyperparams.algorithm

    # todo: handle the case of "her" wrapper.



    # for the algorithms ppo2,sac and td3 we might have to create linear scheduler for the following fields
    # learning_rate, cliprange, cliprange_vf
    # note that the below assumes the config file defines the schedule using string and not directly defines the
    # function
    if algo_ in ["ppo2", "sac", "td3"]:
        for key in ['learning_rate', 'cliprange', 'cliprange_vf']:
            if key not in agent_hyperparams:
                continue
            if isinstance(agent_hyperparams[key], str):
                schedule, initial_value = agent_hyperparams[key].split('_')
                initial_value = float(initial_value)
                agent_hyperparams[key] = linear_schedule(initial_value)
            elif isinstance(agent_hyperparams[key], (float, int)):
                # Negative value: ignore (ex: for clipping)
                if agent_hyperparams[key] < 0:
                    continue
                agent_hyperparams[key] = constfn(float(agent_hyperparams[key]))
            else:
                raise ValueError('Invalid value for {}: {}'.format(key, agent_hyperparams[key]))
...
def main():
    ...
    exp_folder_name = args.exparams + '-' + strftime("%d-%m-%Y_%H-%M-%S")
